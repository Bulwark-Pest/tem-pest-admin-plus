...
function checkLink(node) {
  if (node.to === "backend-training-center" && ENV.SHOW_TRAINING_CENTER === false) return false;
  return !node.heading && !node.sub;
}
</script>
...
<template>
  <ul :class="classContainer">
    ...
    >
      ...
      <!-- Normal Link -->
      <div v-if="checkLink(node)" @click="linkClicked($event)">
        <RouterLink
          :to="node.to && node.to !== '#' ? { name: node.to, params: node.params || {} } : '#'"
          class="nav-main-link"
